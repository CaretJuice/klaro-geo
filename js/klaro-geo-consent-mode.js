...
(function() {
    ...
    function checkGtagAvailable() {
        // If we've already determined gtag is available, return true
        if (gtagAvailable) {
            console.log('DEBUG: gtag was previously marked as available');
            return true;
        }
        ...
        if (window.dataLayer && Array.isArray(window.dataLayer)) {
            ...
            if (typeof window.gtag === 'function') {
                console.log('DEBUG: gtag is available');
                gtagAvailable = true;
                return true;
            } else {
                console.log('DEBUG: dataLayer exists but gtag is not a function');
                
                // Log dataLayer contents for debugging
                console.log('DEBUG: dataLayer contents:', JSON.stringify(window.dataLayer));
            }
        } else {
    ...
    function safeGtag() {
        // Get the call stack to help identify where this function is being called from
        const stack = new Error().stack;
        console.log('DEBUG: safeGtag called from:', stack);
        
        // Log the arguments
        console.log('DEBUG: safeGtag arguments:', Array.from(arguments));
        ...
        // Check if gtag is available
        if (typeof window.gtag !== 'function') {
            console.log('DEBUG: gtag not available, skipping call');
            return false;
        }
        
        try {
            window.gtag.apply(null, arguments);
            console.log('DEBUG: gtag call successful');
            return true;
        } catch (e) {
    ...
    function logGtagStatus() {
        console.log('DEBUG: Checking gtag status');
        console.log('DEBUG: gtag defined:', typeof window.gtag !== 'undefined');
        ...
        if (window.dataLayer && Array.isArray(window.dataLayer)) {
            console.log('DEBUG: dataLayer exists with', window.dataLayer.length, 'items');
        } else {
            console.log('DEBUG: dataLayer does not exist or is not an array');
        }
    }
            ...
            if (typeof window.klaro !== 'undefined') {
                ...
                if (typeof window.klaro.getManager === 'function') {
                    ...
                    try {
                        ...
                        console.log('DEBUG: Before getManager - gtag defined:', typeof window.gtag !== 'undefined');
                        if (typeof window.gtag !== 'undefined') {
                            console.log('DEBUG: gtag is a function:', typeof window.gtag === 'function');
                        }
                        ...
                        console.log('DEBUG: After getManager - gtag defined:', typeof window.gtag !== 'undefined');
                        if (typeof window.gtag !== 'undefined') {
                            console.log('DEBUG: gtag is a function:', typeof window.gtag === 'function');
                        }
            ...
            mutations.forEach(function(mutation) {
                if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                    for (let i = 0; i < mutation.addedNodes.length; i++) {
                        ...
                        if (node.nodeType === 1 && (
                            node.id === 'klaro' || 
                            (node.classList && node.classList.contains('klaro'))
                        )) {
                            console.log('DEBUG: Klaro modal detected in DOM via MutationObserver');
    ...
    function updateConsentState() {
        ...
        // Log the current state of gtag
        console.log('DEBUG: In updateConsentState - gtag defined:', typeof window.gtag !== 'undefined');
        if (typeof window.gtag !== 'undefined') {
            console.log('DEBUG: gtag is a function:', typeof window.gtag === 'function');
        }
        ...
        console.log('DEBUG: Updating gtag consent with:');
        console.log('DEBUG: - ad_storage:', isServiceEnabled ? 'granted' : 'denied');
        console.log('DEBUG: - ad_personalization:', adPersonalizationConsent ? 'granted' : 'denied');
        console.log('DEBUG: - ad_user_data:', adUserDataConsent ? 'granted' : 'denied');

        // Update the consent state
        try {
            window.gtag('consent', 'update', {
                'ad_storage': isServiceEnabled ? 'granted' : 'denied',
                'ad_personalization': adPersonalizationConsent ? 'granted' : 'denied',
                'ad_user_data': adUserDataConsent ? 'granted' : 'denied'
            });
            console.log('DEBUG: Consent state updated in gtag');
        } catch (e) {
            console.error('DEBUG: Error updating consent state in gtag:', e);
        }
    }
